...
| `.github/workflows/ci.yml` | 在 Python 3.11/3.13 上运行编译、覆盖率测试和检索评测 | 是 |
...
| `src/finagent/evaluation.py` | golden questions 的确定性 Hit@K 检索评测 | 是 |
...
腾讯财经 K 线接口单次返回有行数上限，因此下载器按自然年请求。每一个 CSV 只有 `date,close,volume` 三列；同名 `.meta.json` 还保存 SHA-256。运行时检查列、数字、日期严格递增、重复日期、正收盘价、非负成交量和 checksum。
...
当前 44 项测试覆盖来源字段、BM25 与 Hit@5、英文金融短语/中文 bigram、市场 checksum、偏好记忆、模型分阶段参数、空响应、验证器不可绕过、数字漂移、SEC recent/history 与不完整退出、Web、CLI 和安全 HTML。真实模型 smoke 记录在 `DEMO_OUTPUTS.md`。
