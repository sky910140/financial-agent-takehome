...
本目录的受检入口是 `DATA_SNAPSHOT.json`。快照日期为 2026-07-12，内容由已提交文件实际计算，不是手工估算：10 份最新 10-K、3,978 个检索片段，以及 3 组 A 股主要指数日频 CSV。
...
来源为腾讯财经 K-line 公开端点，请求参数按自然年使用 `day` 和 `qfq`。CSV 只保存来源返回的交易日期、收盘价和成交量，未填补非交易日，成交量保持来源原始单位、不做跨指数归一化。各 `.meta.json` 保留全部年度请求 URL、UTC 下载时间、覆盖日期、行数、字段定义和 CSV SHA-256。
