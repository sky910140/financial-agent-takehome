...
- 数据行数和覆盖日期；
- 字段含义；
- 日频、不填补非交易日、`qfq` 请求参数、交易日期时区和 source-native 成交量口径；
- 披露值、确定性计算值和模型解释的边界；
- LF 规范化文本的 CSV SHA-256 校验值；Windows CRLF 与 Linux LF checkout 使用同一口径。

程序运行时会拒绝缺失字段、非法数字、NaN/Inf、重复或乱序日期、非正收盘价、负成交量以及校验值不一致的数据。CSV 中的收盘价和成交量是来源披露值；区间涨跌幅和平均成交量由 Python 程序确定性计算；模型只能解释这些值，不参与算术。
